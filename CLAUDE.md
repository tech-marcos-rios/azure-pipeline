...
- [x] **DNS cargado y certificado emitido** (2026-09-14) — registro `A` `ops` → `2.29.23.254` agregado por Marcos en Cloudflare. Confirmado `nslookup` resolviendo, y en los logs de Caddy: `authorization finalized... valid` → `certificate obtained successfully` (Let's Encrypt producción, no staging). `curl` externo sin credenciales devuelve `401` (Basic Auth activo y bloqueando). Login real con las credenciales pendiente de que Marcos lo confirme desde el browser — no se pudo probar por curl en esta sesión (el clasificador de auto mode bloquea poner la password en texto plano en un comando).
- [x] **`docs/INFRAESTRUCTURA.md` actualizado** (2026-09-14) — agregado a la tabla de proyectos deployados, nota explicando por qué 06 es la excepción al patrón Vercel+Hetzner del resto. De paso corrigió una línea desactualizada de ese doc (`ufw` decía puertos abiertos que ya no lo están — verificado en vivo, no vale la pena repetir el detalle acá, ver ese archivo).

Con esto, **el proyecto 06 (Portfolio Ops Dashboard) queda completo y deployado en producción**, sin pendientes conocidos salvo mejoras futuras (histórico de uptime, sumar `p-aeon`, logs centralizados — todos fuera de alcance de la v1 a propósito, ver arriba).
